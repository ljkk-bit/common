#!/bin/bash
# https://github.com/ljkk-bit/build-actions
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/ljkk-bit/common build/common
...
git clone -b main --depth 1 https://github.com/ljkk-bit/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (ljkk-bit/build-actions|ljkk-bit/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/ljkk-bit/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/ljkk-bit/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="ljkk-bit/build-actions"
  sudo rm -rf build common.sh
